...
async def test_save_inserts_new_auth_account(auth_account_repo):
    user_id = uuid4()
...
async def test_save_updates_existing_auth_account(auth_account_repo):
    user_id = uuid4()
    auth_provider = "google"
    auth_provider_user_id = uuid4().hex
    auth_account = AuthAccount.create(user_id, auth_provider, auth_provider_user_id)
...
async def test_get_auth_accounts_by_user_id(auth_account_repo):
    user_id = uuid4()
    auth_provider1 = "google"
    auth_provider_user_id1 = uuid4().hex
    auth_account1 = AuthAccount.create(user_id, auth_provider1, auth_provider_user_id1)

    auth_provider2 = "github"
    auth_provider_user_id2 = uuid4().hex
    auth_account2 = AuthAccount.create(user_id, auth_provider2, auth_provider_user_id2)

    await auth_account_repo.save(auth_account1)
    await auth_account_repo.save(auth_account2)

    get_auth_accounts = await auth_account_repo.get_by_user_id(user_id)

    assert isinstance(get_auth_accounts, list)
    assert len(get_auth_accounts) == 2


async def test_get_auth_account_by_auth_provider(auth_account_repo):
    user_id = uuid4()
    auth_provider = "google"
    auth_provider_user_id = uuid4().hex
    auth_account = AuthAccount.create(user_id, auth_provider, auth_provider_user_id)
...
async def test_delete_auth_account(auth_account_repo):
    user_id = uuid4()
    auth_provider = "google"
    auth_provider_user_id = uuid4().hex
    auth_account = AuthAccount.create(user_id, auth_provider, auth_provider_user_id)
