...
class AuthUtils:
    ...
    async def decode_token(token: str) -> Token:
        ...
        try:
            ...
            if decoded_token.claims.get("iss") != config.APP_NAME:
                ...
                raise InvalidCredentialsError("invalid token iss")
            ...
            raise InvalidCredentialsError("token expired") from e

        except DecodeError as e:
            StructuredLogger.warning("auth.decode_token.decode_error", error=e)
            raise InvalidCredentialsError("failed to decode token") from e

        except BadSignatureError as e:
            StructuredLogger.warning("auth.decode_token.bad_signature", error=str(e))
            raise InvalidCredentialsError("bad token signature") from e
    ...
    async def get_user_id_from_token(token: str) -> UUID:
        ...
        if not user_id:
            ...
            raise InvalidCredentialsError("failed to get user id")
