...
class InMemoryAuthAccountRepository(BaseAuthAccountRepository):
    ...
    async def get_by_id(self, account_id: UUID) -> AuthAccount | None:
        return next((acc for acc in self.inm_storage.auth_accounts if acc.id == account_id), None)
    ...
    async def get_by_provider(
        ...
        return next((acc for acc in self.inm_storage.auth_accounts if acc.provider == provider and acc.provider_user_id == provider_user_id), None)

    async def save(self, auth_account: AuthAccount) -> None:
        existing_index = next((i for i, acc in enumerate(self.inm_storage.auth_accounts) if acc.id == auth_account.id), None)
    ...
    async def delete(self, account_id: UUID) -> None:
        self.inm_storage.auth_accounts = [account for account in self.inm_storage.auth_accounts if account.id != account_id]
